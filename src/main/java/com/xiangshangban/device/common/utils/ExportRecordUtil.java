            ...
            switch (flag){
                ...
                case 2://TODO 签到签退
                    ...
                    for (int c = 0; c < exportData.size(); c++) {
                        ...
                        //创建字符串数组，保存每一行表格中的内容
                        String[] signInOutCellContent = {sign.getEmpId(),sign.getEmpName(),sign.getEmpDept(),signInArr[0],signInArr[1]+"/"+signOutArr[1]};
                        ...
                        for(int z=0;z<headers.length;z++){
                         ...
                         /*   cell.setCellStyle(contentStyle);*/
                            //设置显示的内容
                            cell.setCellValue(signInOutCellContent[z]);
                        }
                    }
                    break;
                case 3:
                    System.out.println("-------《导出操作日志》------");
                    for (int d = 0; d < exportData.size(); d++) {
                        Map operateLog = (Map)exportData.get(d);
                        //从表格的第二行开始
                        row = sheet.createRow(d + 1);

                        //创建字符串数组，保存每一行表格中的内容
                        String[] operateLogCellContent = {operateLog.get("employee_department_name").toString().trim(),
                                operateLog.get("employee_name").toString().trim(),
                                operateLog.get("operate_type").toString().trim(),
                                operateLog.get("operate_content").toString().trim(),
                                operateLog.get("operate_date").toString().trim()};
                        ...
                        for(int k=0;k<headers.length;k++){
                            ...
                            /*   cell.setCellStyle(contentStyle);*/
                            //设置显示的内容
                            cell.setCellValue(operateLogCellContent[k]);
                        }
                    }
                    break;
                    default:
