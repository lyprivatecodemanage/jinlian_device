...
//        }
     ...
     */
    @Override
    public boolean delDoorInfo(String doorId) {
        int i = doorMapper.delDoor(doorId);
        if(i>0){
            return true;
        }else{
            return false;
     ...
     */
    @Override
    public boolean clearDeviceLog(String deviceId) {
        int result = doorCmdMapper.removeDeviceCommand(deviceId);
        if(result>0){
            return  true;
        }else {
            return false;
    ...
    public List<Map> queryRelateEmpPermissionInfo(Map relateEmpPermissionCondition,String companyId) {
        ...
        //TODO 维护door_employee表进行door_employee表数据的删除
        ...
        //遍历所有人员信息，查询指定设备指定人员相关的指令的最后下发时间和状态
        if(maps!=null && maps.size()>0){
            for(int i=0;i<maps.size();i++){
                Object employee_id = maps.get(i).get("employee_id");
                Object device_id = maps.get(i).get("device_id");
                if(employee_id!=null && device_id!=null){
                    Map requestParam = new HashMap();
                    requestParam.put("employeeId",employee_id.toString().trim());
                   //使用门关联过的所有设备的ID
                    requestParam.put("deviceId",device_id.toString().trim());
                    //TODO 获取下发时间
        ...
        if(doorObj!=null){
            ...
            for(int f=0;f<maps.size();f++){
                ...
                if(DateUtils.isTime1LtTime2(createDate, bindDate)){
                    ...
                    maps.get(f).put("isHistoryDevice","0");
                }
            }
        }
    ...
    //
    @Override
    public void doorCommonSetupAdditionalCmd(String deviceId, String operatorEmployeeId, Map<String, Object> doorSetupMap){
        //构造命令格式
        ...
        doorCmdEmployeeInformation.setData(JSON.toJSONString(doorSetupMap));

        //获取完整的数据加协议封装格式
        RabbitMQSender rabbitMQSender = new RabbitMQSender();
        Map<String, Object> userInformationAll =  CmdUtil.messagePackaging(doorCmdEmployeeInformation, "attSetting", doorSetupMap, "C");
        //命令状态设置为: 发送中
...
//        System.out.println("employeeIdList = "+JSON.toJSONString(employeeIdList));
        ...
        //删除不在新列表中的人员权限
        if("1".equals(enableFirstCardKeepOpen)){
...
//        System.out.println(JSON.toJSONString(oneWeekTimeFirstCardListTemp));
    ...
    //下发首卡常开命令
    @Override
    public void handOutFirstCardCmd(String deviceId, String operatorEmployeeId, List<String> employeeIdList, List oneWeekTimeFirstCardListTemp) {
        //下发的门禁配置DATA数据结构
        Map<String, Object> firstCardSetupMap = new LinkedHashMap<String, Object>();
        firstCardSetupMap.put("employeeIdList", employeeIdList);
        firstCardSetupMap.put("oneWeekTimeList", oneWeekTimeFirstCardListTemp);
        ...
        //获取完整的数据加协议封装格式
        RabbitMQSender rabbitMQSender = new RabbitMQSender();
        Map<String, Object> userInformationAll =  CmdUtil.messagePackaging(doorCmdEmployeeInformation, "firstCardKeepDoorOpen", firstCardSetupMap, "C");
        //命令状态设置为: 发送中
        doorCmdEmployeeInformation.setStatus("1");
        //设置md5校验值
        doorCmdEmployeeInformation.setMd5Check((String) userInformationAll.get("MD5Check"));
...
//        System.out.println("userInformationAll"+JSON.toJSONString(userInformationAll));
    ...
    @Override
    public void handOutDoorCalendarCmd(String deviceId, String operatorEmployeeId, List accessCalendar){
        //构造命令格式
        DoorCmd doorCmdEmployeeInformation = new DoorCmd();
        doorCmdEmployeeInformation.setServerId(serverId);
        doorCmdEmployeeInformation.setDeviceId(deviceId);
        ...
        //获取完整的数据加协议封装格式
        RabbitMQSender rabbitMQSender = new RabbitMQSender();
        Map<String, Object> userInformationAll = CmdUtil.messagePackaging(doorCmdEmployeeInformation, "accessCalendar", accessCalendar, "C");
        //命令状态设置为: 发送中
        doorCmdEmployeeInformation.setStatus("1");
        //设置md5校验值
        doorCmdEmployeeInformation.setMd5Check((String) userInformationAll.get("MD5Check"));
...
//        System.out.println("门禁记录上传已回复MQ");
    ...
    @Override
    public ReturnData handOutDoorFeaturesSetup(String doorFeaturesSetup, String operatorEmployeeId) {
        ...
        try {
            ...
            loginEmployeeId = operatorEmployeeId;
